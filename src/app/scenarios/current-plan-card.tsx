...
export default function CurrentPlanCard() {
  ...
  return (
    <div className="w-full rounded-[20px] bg-white p-4 shadow-[0_8px_24px_rgba(25,25,20,0.05)]">
      <div className="flex flex-col gap-3">
        <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8a8d87]">
          Team access
        </div>

        <div className="flex items-center gap-2 text-sm font-semibold text-[#181815] truncate">
          <Building2 className="h-4 w-4 text-[#1f4d38]" />
          <span className="truncate text-[#1f4d38]">
            {team.companyName || 'Team workspace'}
          </span>
        </div>
        <div className="flex items-center gap-2 text-xs text-[#666864]">
          <UserRound className="h-3.5 w-3.5 text-[#d6612d]" />
          {formatRole(team.role)} · {active ? 'Active plan' : 'Billing required'}
        </div>

        {(isOwner || isAdmin) && (
          <div className="text-xs text-[#666864]">
            Expires:{' '}
            <span className="font-semibold text-[#d6612d]">
        ...
        {isRep && (
          <div className="text-xs text-[#666864]">
            You are part of this workspace.
          </div>
        )}
        {/* {(isOwner || isAdmin) && (
          <button className="mt-2 w-full rounded-full bg-[#1f4d38] px-3 py-2 text-xs font-semibold text-white">
            Dashboard
          </button>
        )} */}
      </div>
